...
void mat4x4_mul_vec4plus(vec4 vOut, mat4x4 M, vec4 vIn, float v3) {
    vec4 v2;
    if (vOut == vIn) {
...
float v3xyLengthFromTo(float* v0, float* v1) {
    float v[3];
    v3fromTo(v, v0, v1);
    float r = v[0] * v[0] + v[1] * v[1];
    if (r == 0)
...
void mylog_v3(std::string vTitle, float* v) {
    mylog("%s%.2f:%.2f:%.2f", vTitle.c_str(), v[0], v[1], v[2]);
}
...
void v3toGL(vec4 vOut, mat4x4 mMVP, vec4 vIn, float v3) {
...
char* v3toStr(float* v) {
    char outStr[256];
    sprintf_s(outStr, 256, "%4.2f : %4.2f : %4.2f", v[0], v[1], v[2]);
    return outStr;
}
