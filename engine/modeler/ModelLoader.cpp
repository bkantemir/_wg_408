...
int ModelLoader::loadModel(std::vector<SceneSubj*>* pSubjsVector0, std::vector<DrawJob*>* pDrawJobs0, std::vector<unsigned int>* pBuffersIds0, 
		std::string sourceFile, std::string subjClass, ProgressBar* pPBar, ModelBuilder* pMB00) {
	...
	for (int subjN0 = totalSubjs - 1; subjN0 >= 0; subjN0--) {
		...
		if (pSS0 == NULL)
			continue;
		if (strcmp(pSS0->source, sourceFile.c_str()) != 0)
			continue;
		if (strcmp(pSS0->className, subjClass.c_str()) != 0)
			continue;
		//if here - model was already loaded - copy
		int subjN = pSubjsVector0->size();
		for (int i = 0; i < pSS0->totalNativeElements; i++) {
			SceneSubj* pSS = pSubjsVector0->at(subjN0 + i)->clone();
			pSS->nInSubjsSet = pSubjsVector0->size();
			pSS->pCustomColors = NULL;
			pSS->pCustomMaterials = NULL;
			pSS->mt0isSet = 0;
			pSS->hide = 0;
			pSubjsVector0->push_back(pSS);
		}
		...
		pSS->d2headTo = 0;
		if(pSS->scale[0] != 1.0)
			pSS->scaleMe(1.0f / pSS->scale[0]);
		strcpy_s(pSS->name, 32, "");
		pSS->pCustomColors = NULL;
		pSS->pCustomMaterials = NULL;
		v3setAll(pSS->ownCoords.pos, 0);
		pSS->ownCoords.setEulerDg(0,0,0);
	...
	pSS->totalElements = pSS->totalNativeElements;
	//pSS->setRoot(pGS);
	buildGabaritesFromDrawJobs(&pSS->gabaritesOnLoad, pDrawJobs0, pSS->djStartN, pSS->djTotalN);
...
int ModelLoader::fillProps_mt(Material* pMT, std::string tagStr, ModelLoader* pML) {
	...
	if (varExists("mt_use", tagStr)) {
		...
		if (!mt_set) {
			MaterialAdjust* pMA = MaterialAdjust::findMaterialAdjust(mt_use.c_str(), NULL);
			if (pMA == NULL) {
	...
	if (pML->tagName.find("mt_adjust") != 0) {
...
int ModelLoader::processTag(ModelLoader* pML) {
	...
	if (pML->tagName.compare("group") == 0) {
		...
		if (varExists("mark", pML->currentTag)) {
			if (getStringValue("mark", pML->currentTag).empty())
				strcpy_s(pMB->pCurrentGroup->marks, 128, "");
			else
	...
	if (pML->tagName.compare("mt_adjust_as") == 0) {
		...
		std::vector<MaterialAdjust*>* pMaterialAdjustsList = &pML->pRootModelBuilder->materialAdjustsList00;
		...
		for (int i = pMaterialAdjustsList->size() - 1; i >= 0; i--)
			if (strcmp(pMaterialAdjustsList->at(i)->materialName, keyName.c_str()) == 0)
				return 0; //name exists
		//add new
		MaterialAdjust* pMtA = new (MaterialAdjust);
		strcpy_s(pMtA->materialName, 32, keyName.c_str());
...
int ModelLoader::processTag_a(ModelLoader* pML) {
	...
	if (strlen(pVS_a->side) > 0) {
		std::vector<std::string>* pSides = splitString(std::string(pVS_a->side), ",");
		for (int sideN = 0; sideN < (int)pSides->size(); sideN++) {
			...
			if (!mtName.empty()) {
				...
				MaterialAdjust* pMA = MaterialAdjust::findMaterialAdjust(mtName.c_str(), &pML->pRootModelBuilder->materialAdjustsList00);
				if (pMA == NULL) {
...
int ModelLoader::processTag_a2mesh(ModelLoader* pML) {
	...
	for (int vN = 0; vN < vxTotal; vN++)
		strcpy_s(vx2.at(vN)->marks, 128, "");
	for (int tN = 0; tN < trTotal; tN++)
		strcpy_s(tr2.at(tN)->marks, 128, "");
...
int ModelLoader::processTag_element(ModelLoader* pML) {
	...
	if (!sourceFile.empty()) {
		sourceFile = buildFullPath(pML, sourceFile);
		subjN = loadModel(pML->pSubjsVector, pML->pDrawJobs, pML->pBuffersIds,
			sourceFile, subjClass, pML->pPBar, pML->pRootModelBuilder);
	}
	...
	setCharsValue(pSS->name, 32, "name", tagStr);

	std::string attachTo = getStringValue("attachTo", tagStr);
	if (attachTo.empty()) //attach to root
		pSS->d2parent = subjN - rootN;
	else {
		//find parent by name
		for (int sN = subjN - 1; sN >= rootN; sN--) {
			if (strcmp(pML->pSubjsVector->at(sN)->name, attachTo.c_str()) == 0) {
	...
	if (!headTo.empty()) { //find headTo by name
		for (int sN = subjN - 1; sN >= rootN; sN--) {
			if (strcmp(pML->pSubjsVector->at(sN)->name, headTo.c_str()) == 0) {
...
int ModelLoader::processTag_include(ModelLoader* pML) {
	...
	pMB->lockGroup(pMB);
	ModelLoader* pML2 = new ModelLoader(pML->pSubjsVector, pML->pDrawJobs, pML->pBuffersIds, subjN, pML->pRootModelBuilder, pMB, sourceFile, pML->pPBar);
	processSource(pML2);
	delete pML2;
	GroupTransform gt;
	fillProps_gt(&gt, pMB, tagStr);
...
int ModelLoader::processTag_group2line(ModelLoader* pML) {
	...
	for (int vN = totalN - 1; vN >= 0; vN--) {
		...
		if (strlen(lineName) > 2)
			if (strstr(pV0->marks, lineName) == NULL)
				continue;
