...
void SceneSubj::buildModelMatrixStandard(SceneSubj* pSS) {
    ...
    pSS->updateFrameN = theApp.frameN;
}
...
int SceneSubj::processSubjStandart(SceneSubj* pSS) {
    ...
    pSS->moveSubj();
    if (pSS->updateFrameN != theApp.frameN)
...
int SceneSubj::executeDJcommon(DrawJob* pDJ, Shader* pShader,
 ...
 /*
    if (pShader->l_uVectorToLight >= 0) {
         mylog("Translucency=%d VectorToLight=%d, VectorToTranslucent=%d, shader %d %s\n", pShader->l_uTranslucency, pShader->l_uVectorToLight, pShader->l_uVectorToTranslucent, pDJ->mt.shaderN, pShader->shaderType);
    }
*/
    if (pShader->l_uTranslucency >= 0) {
        ...
        if (pMt->uTranslucency > 0) {
           ...
           // mylog("uVectorToLight=%s, uVectorToTranslucent=%s\n", v3toStr(uVectorToLight),v3toStr(uVectorToTranslucent));
        }
    }
...
int SceneSubj::getScreenPosition(SceneSubj* pSS, mat4x4 mViewProjection) { //, Camera* pCam, float* dirToMainLight) {
    ...
    */
    ...
    float vOut[4];
    for (int z = -1; z <= 1; z += 2) {
        vIn[2] = pSS->gabaritesOnLoad.bbMid[2] + pSS->gabaritesOnLoad.bbRad[2] * z;
        for (int y = -1; y <= 1; y += 2) {
            vIn[1] = pSS->gabaritesOnLoad.bbMid[1] + pSS->gabaritesOnLoad.bbRad[1] * y;
            for (int x = -1; x <= 1; x += 2) {
                vIn[0] = pSS->gabaritesOnLoad.bbMid[0] + pSS->gabaritesOnLoad.bbRad[0] * x;
                v3toGL(vOut, mMVP, vIn, 1);
                for (int i = 0; i < 3; i++) {
                    float xx = vOut[i];
                    if (pSS->gabaritesOnScreenGL.bbMin[i] > xx)
    ...
    int score = 0;
    for (int i = 0; i < 3; i++) {
        if (pSS->gabaritesOnScreenGL.bbMin[i] > 1.0f) {
            score = -1;
            break;
        }
        if (pSS->gabaritesOnScreenGL.bbMax[i] < -1.0f) {
            score = -1;
            break;
        }
        if (pSS->gabaritesOnScreenGL.bbMin[i] > -1.0f)
    ...
    /*
if (pSS->nInSubjsSet == 1) {
    mylog("min %f/%f/%f max %f/%f/%f score=%d isOnScreen=%d\n",
        pSS->gabaritesOnScreenGL.bbMin[0],
        pSS->gabaritesOnScreenGL.bbMin[1],
        pSS->gabaritesOnScreenGL.bbMin[2],
        pSS->gabaritesOnScreenGL.bbMax[0],
        pSS->gabaritesOnScreenGL.bbMax[1],
        pSS->gabaritesOnScreenGL.bbMax[2],
        score,
        pSS->isOnScreen);
...
*/
    ...
    float biggestSize = 0;
    for (int axisN = 0; axisN < 3; axisN++) {
        if (biggestSize >= sizeXY[axisN])
            continue;
        biggestSize = sizeXY[axisN];
        biggestAxisN = axisN;
    }
    float* pDirChord = sizeDirXY[biggestAxisN];
    //perpendicular
    float pDirPerp[2];
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (sizeXY[i] == 0)
            continue;
        chordH = chordH + abs(v2dotProductNorm(pDirChord, sizeDirXY[i])) * sizeXY[i];
        chordR = chordR + abs(v2dotProductNorm(pDirPerp, sizeDirXY[i])) * sizeXY[i];
    }
    pSS->gabaritesOnScreenGL.chordR = chordR;
    chordH -= chordR;
    if (chordH < 1.0f) //dot
        pSS->gabaritesOnScreenGL.chord.initLineXY(pSS->gabaritesOnScreenGL.bbMid, pSS->gabaritesOnScreenGL.bbMid);
    else {
        float dot0[2];
        float dot1[2];
        for (int i = 0; i < 2; i++) {
        ...
        pSS->gabaritesOnScreenGL.chord.initLineXY(dot0, dot1);
    }
    */
    return 1;
}
